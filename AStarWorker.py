...
def findPath(start,goal):
    ...
    openList = []
    closedList = []
    currentNode = Node(start,None,0,getDistance(start,goal))
    ...
    while len(openList) >0:
        ...
        for i in range(9):
            ...
            if tile.isSolid:
                ...
                continue
            tilePos = (x+dx,y+dy)
            costSoFar = currentNode.costSoFar + getDistance(currentNode.pos,tilePos)
            costSoFar*=tile.getSpeed()
            distanceToEnd = getDistance(tilePos,goal)
            node = Node(tilePos,currentNode,costSoFar,distanceToEnd)
            if inList(closedList,tilePos) and costSoFar >= node.costSoFar: #checks if node has already been used,or if you are going backwards
                continue
            if not inList(openList,tilePos) or costSoFar < node.costSoFar:#only add if it's not alredy there
